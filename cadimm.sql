...
--
-- Host: 127.0.0.1
-- Tempo de geração: 06-Set-2019 às 23:34
...
INSERT INTO `adm` (`nome`, `cpf`) VALUES
('LUCIANA AFONSO DO NASCIMENTO DUTRA', '2e0642e0917690d89f7f3948c03ed3dc'),
('GLEIDISON DA SILVA DE ABREU', '66951107a8b78c7f21decc3e877d02c2');
...
CREATE TABLE `endereco` (
  ...
  `cep` char(8) NOT NULL,
  `endereco_usuario` char(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `familia` (
  `pai` char(80) NOT NULL,
  `mae` char(80) NOT NULL,
  `conjugue` char(80) DEFAULT NULL,
  `telConjugue` char(17) DEFAULT NULL,
  `familia_usuario` char(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `usuario` (
  ...
  `escolaridade` char(50) NOT NULL,
  `estadoCivil` char(13) DEFAULT NULL,
  `filiacao` char(80) NOT NULL,
  `consagracaoPretendida` char(30) DEFAULT NULL,
  `consagracaoAtual` char(30) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
ALTER TABLE `cidade`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fkCidade_Estado` (`estado`);

--
-- Índices para tabela `endereco`
--
ALTER TABLE `endereco`
  ADD KEY `endereco_usuario` (`endereco_usuario`) USING BTREE;
...
ALTER TABLE `estado`
  ADD PRIMARY KEY (`id`);
ALTER TABLE `estado` ADD FULLTEXT KEY `nome` (`nome`);

--
-- Índices para tabela `familia`
--
ALTER TABLE `familia`
  ADD KEY `familia_usuario` (`familia_usuario`) USING BTREE;

--
-- Índices para tabela `perfil`
--
ALTER TABLE `perfil`
  ADD KEY `perfil_usuario` (`perfil_usuario`) USING BTREE;
...
--
ALTER TABLE `cidade`
  ADD CONSTRAINT `fkCidade_Estado` FOREIGN KEY (`estado`) REFERENCES `estado` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Limitadores para a tabela `endereco`
--
ALTER TABLE `endereco`
  ADD CONSTRAINT `endereço_fk` FOREIGN KEY (`endereco_usuario`) REFERENCES `usuario` (`cpf`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Limitadores para a tabela `familia`
--
ALTER TABLE `familia`
  ADD CONSTRAINT `familia_fk` FOREIGN KEY (`familia_usuario`) REFERENCES `usuario` (`cpf`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Limitadores para a tabela `perfil`
--
ALTER TABLE `perfil`
  ADD CONSTRAINT `perfil_fk` FOREIGN KEY (`perfil_usuario`) REFERENCES `usuario` (`cpf`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
